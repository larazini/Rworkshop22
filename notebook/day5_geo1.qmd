...
-   [What is Geographic Information System's (GIS)?](#what-is-geographic-information-systems-gis)
...
-   [What is a Coordinate Reference Systems (CRS):](#what-is-a-coordinate-reference-systems-crs)

    -   [Diferent types of Coordinate reference Systems (CRS)](#diferent-types-of-coordinate-reference-systems-crs)
...
| Spatial Databases | PostGIS / PostgreSQL (con esto podes manejar spatial databases en SQL) |                                     |
...
library(tidyr)
library(sf)
library(albersusa) #tiene los limites, poligonos y eso de USA
library(ggplot2)
library(mapview)
library(tidyverse)
library(urbnmapr)
...
#en una columna que se llama geometria esta la data del poligono. No tiene una columna de latitud y longitud
```
...
ggplot() + 
  geom_sf(data = kansas_counties) #con esto ploteo Kansas
...
```{r}
mesonet_sites <- read.csv('../data/Geo_Class1/Geo_Class1/KSMeso-Station-Metadata.csv',skip = 2)
mesonet_sites
```
...
```{r}
#LEO UN SHAPEFILE
zones <- read_sf('../data/Geo_Class1/Geo_Class1/management_zones.shp')

seeding_rate_trial <- read_sf('../data/Geo_Class1/Geo_Class1/corn_density_trial.shp')
```

```{r} # ESTO ME SIRVE PARA VER LO QUE TENGO ADENTRO DEL ARCHIVO}
zones
...
```{r}
st_crs(zones) #PARA SABER QUE COORDENADAS TENGO

```

```{r}
st_crs(seeding_rate_trial) #PARA SABER QUE COORDENADAS TIENE
```

```{r}
#UNA VEZ QUE SE EL TIPO DE COORDENADA, SE QUE ES WGS84 EPSG TANTO.. ENTONCES TRANSFORMO A METRICAS
zones <- zones %>%
  st_set_crs(st_crs("epsg:3857")) %>%
  st_transform(crs=32614) %>%
  mutate(Zone = factor(Zone, levels=c("Low", "Medium", "High")), #DETERMINO LEVELS DE CADA ZONA, EN UNA NUEVA COLUMNA
         area_ha = units::set_units(st_area(zones),ha)) #ACA HAGO OTRA COLUMNA PARA CALCULAR EL AREA.
...
#LOS PARAMETROS USADOS ACA SON DE REGRETION MODEL, Y=B0+B1.X+B2.X^2
